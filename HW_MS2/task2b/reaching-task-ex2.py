...
pertubations_interference = [' sudden \n perturbation', ' no \n perturbation', ' no pertubation \n target shift', ' interference', ' no perturbation \n target shift', ' no \n perturbation',' sudden \n perturbation', ' no \n perturbation']
...
if exp_setup == 'interference':
    ...
    if test_mode:
        ...
        timestamps_changes = [1,20,40,60,80,100,120,140,160,180]
        timestamps_changes_full = [0,20,40,60,80,100,120,140,160,181]
    ATTEMPTS_LIMIT = timestamps_changes[len(timestamps_changes)-1]+1
...
while running:
    ...
    if exp_setup == 'perturbation_types': 
        ...
        if attempts == 1:
            ...
            pertubation_mode = True
            pertubation_type = 'sudden'
            perturbation_angle = math.radians(60)
        elif attempts == timestamps_changes[2]:
            pertubation_mode = False
        elif attempts == timestamps_changes[3]:
            pertubation_mode = False
            start_target=math.radians(-140)
        elif attempts == timestamps_changes[4]:
            pertubation_mode = True
            pertubation_type = 'sudden'
            perturbation_angle = math.radians(-40)
        elif attempts == timestamps_changes[5]:
            pertubation_mode = False
        elif attempts == timestamps_changes[6]:
            pertubation_mode = False
...
print(error_angles)
np.savetxt("task2B2.csv",error_angles,delimiter =", ")
