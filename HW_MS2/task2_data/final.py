...
start_time = 0
exp_setup = 'baseline' 
feedback_type = 'traj'  #'endpos'  'traj'   'rl'
cursor = False
...
while running:
    ...
    if attempts > -1 and not math.hypot(circle_pos[0] - START_POSITION[0], circle_pos[1] - START_POSITION[1]) > TARGET_RADIUS and new_target:
        if feedback_type == 'traj':
            trajectory.append(circle_pos)
    elif attempts > -1 and not math.hypot(circle_pos[0] - START_POSITION[0], circle_pos[1] - START_POSITION[1]) > TARGET_RADIUS and not new_target:
        cursor = True
    ...
    if check_target_reached():
        ...
        #if math.hypot(circle_pos[0] - START_POSITION[0], circle_pos[1] - START_POSITION[1]) < TARGET_RADIUS*1.01:
            #feedback_circle = circle_pos
        a = generate_endpos(error_angle)
    ...
    #miss if player leaves the target_radius + 1% tolerance
        ...
        error_angle = calculate_angle(START_POSITION[0],START_POSITION[1],targetX,targetY,circle_pos[0],circle_pos[1]) #np.arcsin((circle_pos[0] - START_POSITION[0])/distance)
        #if math.hypot(circle_pos[0] - START_POSITION[0], circle_pos[1] - START_POSITION[1]) > TARGET_RADIUS:
         #   feedback_circle = circle_pos
...
np.savetxt('time.csv', time_per_trial, delimiter=',')
...
div = 1000
time_per_trial = [val/div for val in time_per_trial]
plt.figure(figsize=(16,8))
for change in range(1, len(timestamps_changes)-1):
    plt.axvline(x=timestamps_changes[change], color='red')
    plt.text(timestamps_changes[change], np.nanmax(time_per_trial)+4, pertubations[change -1], color = 'red',rotation=0, va='top')

plt.scatter(np.arange(0,len(time_per_trial)),time_per_trial)
plt.xticks(np.arange(0,len(time_per_trial)))
plt.xlabel('trial #')
plt.ylabel('time in s')
plt.ylim(0,len(time_per_trial)+1)
