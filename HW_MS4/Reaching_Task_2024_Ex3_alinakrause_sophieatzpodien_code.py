...
while running:
    ...
    #miss if player leaves the target_radius + 1% tolerance
        ...
        if feedback_type == 'rl_gradual':
            if abs(error_angle) > 60 and abs(error_angle)<15:
                start_pos_color = RED
            elif abs(error_angle) >15 and abs(error_angle < 60): 
                start_pos_color = (230,255-(255 / 60)*abs(error_angle),0)
            elif not (abs(error_angle) > 15):
                start_pos_color = (255-(255/15)*abs(error_angle),155,0)
            if not COLORMAP:
                start_pos_color = RED

        if feedback_type == 'rl_step':
            if abs(error_angle) < 10:
                start_pos_color = GREEN
                print('error:',error_angle,' green')
            elif abs(error_angle) < 35:
                print('error:',error_angle,' yellow')
                start_pos_color = YELLOW
            elif abs(error_angle) < 50:
                print('error:',error_angle,' orange')
                start_pos_color = ORANGE
            else:
                print('error:',error_angle,' red')
                start_pos_color = RED
